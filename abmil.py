...
class ABMIL(MIL):
    ...
    def forward_attention(self, h: torch.Tensor, attn_mask=None, attn_only=True):
        if self.debug:
            print("\n[ABMIL] forward_attention")
            print(f"  input h: {h.shape}, dtype={h.dtype}, device={h.device}")

        h = self.patch_embed(h)

        if self.debug:
            print(f"  after patch_embed: {h.shape}")
            print(f"  h stats: min={h.min().item():.4f}, max={h.max().item():.4f}")

        A = self.global_attn(h)  # B x M x K

        if self.debug:
            print(f"  raw attention A: {A.shape}")

        A = torch.transpose(A, -2, -1)  # B x K x M

        if attn_mask is not None:
            if self.debug:
                print(f"  attn_mask: {attn_mask.shape}, unique={attn_mask.unique()}")
            A = A + (1 - attn_mask).unsqueeze(dim=1) * torch.finfo(A.dtype).min

        if self.debug:
            print(f"  final A (before softmax): {A.shape}")

        return A if attn_only else (h, A)

    def forward_features(self, h, attn_mask=None, return_attention=True):
        if self.debug:
            print("\n[ABMIL] forward_features")
            print(f"  input h: {h.shape}")

        h, A_base = self.forward_attention(h, attn_mask=attn_mask, attn_only=False)

        if self.debug:
            print(f"  A_base: {A_base.shape}")
            print(f"  A_base stats: min={A_base.min().item():.4f}, max={A_base.max().item():.4f}")

        A = F.softmax(A_base, dim=-1)

        if self.debug:
            print(f"  A (softmax): {A.shape}")
            print(f"  A sum (should be 1): {A.sum(dim=-1)}")

        h = torch.bmm(A, h).squeeze(dim=1)

        if self.debug:
            print(f"  bag features h: {h.shape}")
    ...
    def forward(self, h, loss_fn=None, label=None,
            attn_mask=None, return_attention=False,
        ...
        if self.debug:
            ...
            if label is not None:
                print(f"label: {label.shape}, dtype={label.dtype}, unique={label.unique()}")
        ...
        if self.debug:
            print(f"loss: {cls_loss.item() if cls_loss is not None else None}")
            print("===============================================\n")
